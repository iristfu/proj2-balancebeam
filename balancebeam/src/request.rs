...
pub fn write_to_stream(
    ...
    request_bytes.extend_from_slice(&format_request_line(request).into_bytes());
    request_bytes.extend_from_slice(&['\r' as u8, '\n' as u8]); // \r\n
    for (header_name, header_value) in request.headers() {
        request_bytes.extend_from_slice(&format!("{}: ", header_name).as_bytes());

...
pub enum Error {
    ...
    // IncompleteResponse, // removed (2022); no longer needed.
...
fn read_headers(stream: &mut TcpStream) -> Result<http::Response<Vec<u8>>, Error> {
    ...
    loop {
        ...
        // This error check was removed (2022), because it led to unreliable behavior. 
	//if new_bytes == 0 {
...
pub fn write_to_stream(
    ...
    // Response line:
    response_bytes.extend_from_slice(format_response_line(response).as_bytes());
    response_bytes.extend_from_slice(&['\r' as u8, '\n' as u8]);
    // Headers:
    for (header_name, header_value) in response.headers() {
        response_bytes.extend_from_slice(&format!("{}: ", header_name).as_bytes());
        response_bytes.extend_from_slice(header_value.as_bytes());
        response_bytes.extend_from_slice(&['\r' as u8, '\n' as u8]);
    }
    // Body:
    response_bytes.extend_from_slice(&['\r' as u8, '\n' as u8]);

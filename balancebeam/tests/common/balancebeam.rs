...
impl BalanceBeam {
    ...
    pub async fn new(
        ...
        cmd.arg("--active-health-check-path").arg("/health-check");
        ...
        if let Some(rate_limit_window_size) = rate_limit_window_size {
            cmd.arg("--rate-limit-window-size")
                .arg(rate_limit_window_size.to_string());
        }
        if let Some(max_requests_per_window) = max_requests_per_window {
            cmd.arg("--max-requests-per-window")
                .arg(max_requests_per_window.to_string());
        }

...
#[derive(Debug)]
struct ServerState {
    pub non_healthcheck_requests_received: atomic::AtomicU64,
}
...
async fn echo(
    ...
    println!("{}", req.uri());
    if &req.uri().to_string() != "/health-check" {
        server_state
            .non_healthcheck_requests_received
...
impl EchoServer {
    ...
    pub async fn new_at_address(bind_addr_string: String) -> EchoServer {
        ...
        let server_state = Arc::new(ServerState {
            non_healthcheck_requests_received: atomic::AtomicU64::new(0),
        });
...
impl Server for EchoServer {
    async fn stop(self: Box<Self>) -> u64 {
        ...
        self.state.non_healthcheck_requests_received.load(atomic::Ordering::SeqCst)
    }
